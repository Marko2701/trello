...
items.forEach(item => item.addEventListener("click", e => openModal(e.target)));
...
const dragNdrop = function () {
  ...
  listItems.forEach(function (item) {
    ...
    //   let tmpPos, tmpList;
    //   if (cards[`${item.id}`]) {
    //     tmpPos = cards[`${item.id}`]["xPos"];
    //     tmpList = cards[`${item.id}`]["xList"];
    //     let newPos = tmpPos;
    //     tmpListLength = lists[tmpList].getElementsByTagName("div").length;
    //     if (tmpListLength > tmpPos + 1) {
    //       for (let card in cards) {
    //         if (cards[card]["xList"] == tmpList) {
    //           if (cards[card]["xPos"] >= tmpPos) {
    //             cards[card]["xPos"] = newPos - 1;
    //             newPos += 1;
    //           }
    //         }
    //       }
    //     }
    //     delete cards[`${item.id}`];
    //     item.remove();
    //   }
    // });
  });
  ...
  lists.forEach(function (container) {
    container.addEventListener("dragover", function (e) {
      ...
      if (afterElement == null) {
        ...
        console.log(item.id);
        cards[item.id]["xPos"] = "newPos";
        cards[item.id]["xList"] = "newList";
      } else {
        container.insertBefore(item, afterElement);
        console.log(item, afterElement);
      }
    });
